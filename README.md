...
| `npm test` | 통과 | 4개 파일, 10개 테스트 통과 |
| `npm run lint` | 기존 오류 잔존 | Node/Service Worker 전역 설정 및 `App.tsx` 규칙 위반 포함 |

현금 비중의 제품 정의는 `예수금 ÷ 자산평가액`입니다. 여기서 자산평가액은 `보유종목 평가금액 + 예수금`이며 구현과 단위 테스트 모두 이 정의를 사용합니다.

국내 ETF의 다음 달 예상 배당금은 `운용사 공식 페이지 → ETF Explorer → 최근 3개월 실제 수령액 평균` 순으로 계산합니다. 공식 페이지 어댑터는 SOL, TIGER, KODEX, ACE, RISE를 대상으로 시도하며 외부 페이지 구조나 응답이 달라지면 다음 데이터원으로 자동 전환합니다. 네이버 금융과 ETF Shopping은 국내 분배금 데이터원으로 사용하지 않습니다.
